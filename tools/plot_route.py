...
vsrc = (site / "stops-data.js").read_text(encoding="utf-8")
vstops = json.loads(vsrc[vsrc.index("=") + 1:].rstrip().rstrip(";"))
...
ax.scatter([v["lng"] for v in vstops], [v["lat"] for v in vstops],
           s=30, marker="s", facecolors="none", edgecolors="m", zorder=6)
ax.set_title("Full route + stops (black) + vehicle stops (magenta)")
ax.set_aspect(1.3)
...
for (title, la0, la1, lo0, lo1), ax in zip(zooms, axes.flat[1:]):
    ...
    ax.scatter([v["lng"] for v in vstops], [v["lat"] for v in vstops],
               s=46, marker="s", facecolors="none", edgecolors="m", zorder=6)
